...
class ImprovedRPPGNetV4(nn.Module):
    def __init__(self):
        super().__init__()
        # Углубленная сеть с большим количеством каналов
        self.net = nn.Sequential(
            nn.Conv1d(3, 64, 7, 1, 3), 
            nn.ReLU(), 
            ResBlock(64, 128, 2), # 600 -> 300
            SEBlock(128),
            ResBlock(128, 256, 2), # 300 -> 150
            SEBlock(256),
            ResBlock(256, 256, 2), # 150 -> 75 (Дополнительный блок)
            SEBlock(256),
...
class FastRPPGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Обрезка/Дополнение до 600 кадров
        if len(sig) > self.window: 
            sig = sig[:self.window]
        else: 
            sig = np.pad(sig, ((0, max(0, self.window - len(sig))), (0, 0)))
        
        # Аугментация шумом (только для обучения)
        if self.train:
            noise = np.random.normal(0, 0.015, sig.shape).astype(np.float32)
            sig = sig + noise
...
def log_message(message, log_file="training_log_v4.txt"):
    print(message)
...
if __name__ == "__main__":
    ...
    with open(log_file, "w") as f:
        f.write(f"=== Training v4 Started: {datetime.datetime.now()} ===\n")

    # Использование флага train=True для аугментации
    train_loader = DataLoader(FastRPPGDataset('train_labels.csv', train=True), batch_size=64, shuffle=True)
    val_loader = DataLoader(FastRPPGDataset('val_labels.csv', train=False), batch_size=64)
    
    model = ImprovedRPPGNetV4().to(device)
    opt = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    
    # Исправленный планировщик (без аргумента verbose)
    ...
    for e in range(num_epochs):
        ...
        if avg_val_mae < best_val_mae:
            ...
            torch.save(model.state_dict(), 'best_rppg_model_v4.pth')
            log_message(f"  --> Saved new best model v4", log_file)